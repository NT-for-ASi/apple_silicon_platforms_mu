/**
 * Copyright (c) 2026, NTASP authors.
 * 
 * Module Name:
 *     AppleDartIoMmuDxe/IoMmu.c
 * 
 * Abstract:
 *     Initializes the EDK2 IOMMU protocol so that devices dependent on the DARTs 
 *     in translation mode can map buffers through the DART (this should just be PCIe)
 *     Based on Microsoft's open-sourced SmmuDxe driver in 202511 MU_BASECORE.
 * 
 * 
 * Environment:
 ...
 * 
 *     Original DART code basis and RE work
 *     is from the Asahi Linux project, 
 *     original copyright and author notices below.
 *     Copyright (C) 2021 Mark Kettenis <kettenis@openbsd.org>
 *     Copyright (C) The Asahi Linux Contributors.
*/

// Original driver basis is Microsoft's SmmuDxe driver, original copyright notice below.
...
typedef struct IOMMU_MAP_INFO {
  UINTN                    NumberOfBytes;
  UINT64                   DeviceAddress; // DART IO virtual address. (What the device thinks its writing to.)
  UINT64                   HostAddress; // actual physical host address the device is writing to.
