# AppleDartIoMmuDxe Driver

This driver is intended to add support for Apple's custom IOMMU hardware, known as a Device Address Resolution Table (or DART). The
driver will configure the DART hardware appropriately, set up and manage the page tables for the hardware, and mediate PCIe DMA transactions.

Note that this driver is primarily intended for now to support devices that *require* the DART to operate in translated mode. (Bypass mode not supported)
To our current knowledge, this is just the PCIe DARTs. While this is the case, we will only need to officially operate the PCIe DARTs with 
translation mode.

For the NTASP project, drivers that depend on a DART that support bypass mode will be expected to configure their DARTs separately to operate in bypass mode. (Ideally, we'd have a way for such drivers to still use this driver so as to avoid code duplication, but that's currently WIP.)

The original SmmuDxe README is provided below for reference while the driver is a work-in-progress.
